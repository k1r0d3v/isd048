...
public class XmlServiceReservationDtoConversor
{
    public final static Namespace XML_NS = Namespace.getNamespace("http://ws.udc.es/reservations/xml");

    public static Document toXml(ServiceReservationDto reservation) throws IOException {
        Element movieElement = toJDOMElement(reservation);
        return new Document(movieElement);
    }

    public static Document toXml(List<ServiceReservationDto> reservations) throws IOException {

        Element showElements = new Element("reservations", XML_NS);
        for (ServiceReservationDto i : reservations)
            showElements.addContent(toJDOMElement(i));

        return new Document(showElements);
    }

    public static Element toJDOMElement(ServiceReservationDto reservation) {
    ...
    public static ServiceReservationDto toServiceShowDto(InputStream movieXml) throws ParsingException {
        try {
    ...
    private static ServiceReservationDto toServiceShowDto(Element reservationElement)
        ...
        return new ServiceReservationDto(identifier, Long.parseLong(showId), email, cardNumber,
